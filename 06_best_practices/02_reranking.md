...
Reranking orders retrieved documents by a better relevance score. It sits at the end of the retrieval pipeline, before generation. 
...
response = es_client.search(
    index=index_name, query=keyword_query, knn=knn_query, size=5, rank={"rrf": {}}
)
...
def elastic_search_hybrid_rrf(field, query, vector, course, k=60):
    """
    Run both vector and text searches:
    """
    knn_query = {
        "field": field,
        "query_vector": vector,
    ...
    knn_response = es_client.search(index=index_name, knn=knn_query, size=5)
    keyword_response = es_client.search(index=index_name, query=keyword_query, size=5)
...
When the same document appears in both search results, its RRF scores are summed, giving it a higher rank.
...
- Hybrid with reranking using RRF: Hit Rate 0.925, MRR 0.851

Reranking improves both metrics by a few percent. The gain is modest here. In production systems with more data and noisier queries,reranking can make a bigger difference.